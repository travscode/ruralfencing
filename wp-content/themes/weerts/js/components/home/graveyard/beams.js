...
function createBeamMaterial(speed, noiseIntensity, scale) {
	...
	// Set custom uniforms
	baseUniforms.diffuse.value = new THREE.Color(...hexToNormalizedRGB('#000000'))
...
class Beams {
	constructor(container, props = {}) {
		// Default props
		this.props = {
			beamWidth: props.beamWidth || 2,
			beamHeight: props.beamHeight || 15,
			beamNumber: props.beamNumber || 12,
			lightColor: props.lightColor || '#ffffff',
			speed: props.speed || 2,
			noiseIntensity: props.noiseIntensity || 1.75,
	...
	init() {
		...
		this.mainGroup.add(this.beamMesh)
	...
	setupLights() {
		...
		// Directional light
		const dirLight = new THREE.DirectionalLight(this.props.lightColor, 1)
		dirLight.position.set(0, 3, 10)

		// Shadow setup (optional, commented out as original didn't show shadow usage)
	...
	animate() {
		...
		// Update time uniform
		this.beamMaterial.uniforms.time.value += 0.1 * 0.016 // Assuming 60fps, delta ≈ 0.016
	...
	updateProps(newProps) {
		...
		if (newProps.speed !== undefined) {
			this.beamMaterial.uniforms.uSpeed.value = newProps.speed
		}
		if (newProps.noiseIntensity !== undefined) {
			this.beamMaterial.uniforms.uNoiseIntensity.value = newProps.noiseIntensity
		}
		if (newProps.scale !== undefined) {
			this.beamMaterial.uniforms.uScale.value = newProps.scale
		}
		if (newProps.rotation !== undefined) {
			this.mainGroup.rotation.z = degToRad(newProps.rotation)
		}
		// For other props like beamWidth, beamHeight, etc., you'd need to recreate geometry
	}
...
export default function initBeams() {
	const container = document.getElementById('beam-container')

	if (!container) return

	const beams = new Beams(container, {
		beamWidth: 3,
		beamHeight: 32,
		beamNumber: 3,
		lightColor: '#ffffff',
