...
<div id="mw_wc_qbo_sync_grph_div" style="background:white;">
	<div class="page_title">
		<div class="dashboard_main_buttons">
			...
			<button title="<?php esc_attr_e( 'Clear all data from map tables', 'myworks-sync-for-xero' );?>" id="mwqs_clear_all_mappings"><?php esc_html_e( 'Clear All Mappings', 'myworks-sync-for-xero' );?>
			</button>
			&nbsp;
			
			<a id="mwqs_refresh_data_from_qbo" href="#">
				<button title="<?php esc_attr_e( 'Refresh your sync to recognize the latest customers and products currently in Xero.', 'myworks-sync-for-xero' );?>">
					<?php esc_html_e( 'Refresh Customers & Products', 'myworks-sync-for-xero' );?>
				</button>
			</a>
...
<div class="dash-bottm mwqs_db_status_cont">
	<div class="col-sm3 module-stat">
		<h3><?php esc_html_e( 'Sync Status', 'myworks-sync-for-xero' );?></h3>
		<ul>
			<li>
			...
			<li>
				<a <?php if(!$dbsd['xero_initial_data_loaded']){echo 'class="dbst_err"';}?> title="Customers / Products">
					<?php esc_html_e( 'Initial Xero Data Loaded', 'myworks-sync-for-xero' );?>
				</a>
			</li>
			
			<li>
				<a <?php if(!$dbsd['default_settings_saved']){echo 'class="dbst_err"';}?>>
					<?php esc_html_e( 'Default Settings Saved', 'myworks-sync-for-xero' );?>
				</a>
			</li>
			
			<li>
				<a <?php if(!$dbsd['basic_mapping_done']){echo 'class="dbst_err"';}?>>
					<?php esc_html_e( 'Mapping Active', 'myworks-sync-for-xero' );?>
				</a>
			</li>
		</ul>
	</div>
...
<script>
	...
	<?php if($MWXS_L->is_xero_connected()):?>
		$('#mwqs_refresh_data_from_qbo').click(function(e){
			...
			if(!confirm('<?php echo esc_js(__('This will update the data in our sync with the latest Customers & Products in your Xero company. No data will be synced at this time.','myworks-sync-for-xero'))?>')){
				return false;
			}
	...
	<?php endif;?>
		...
		$('#mwqs_clear_all_mappings').click(function(){
			if(confirm('<?php echo esc_js(__('Are you sure you want to clear your mappings?','myworks-sync-for-xero'))?>')){
