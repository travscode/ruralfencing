...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_variation', 'map_wc_xero_variation' ) ) {	
	...
	$wpdb->query("DELETE FROM `" . esc_sql($table) . "` WHERE `X_P_ID` = '' "); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	$wpdb->query("DELETE FROM `" . esc_sql($m_table) . "` WHERE `wc_type` = 'variation' AND `x_type` = 'account' AND `x_id` = '' "); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	<div class="card">
		<div class="card-content">
			<div class="row">
				...
				<div class="col col-m">
					<h5><?php esc_html_e( 'Clear all variation mappings', 'myworks-sync-for-xero' );?></h5>
					<?php wp_nonce_field( 'myworks_wc_xero_sync_clear_variation_mappings', 'clear_variation_mappings' ); ?>
					<button id="mwqs_cavm_btn"><?php esc_html_e( 'Clear Mappings', 'myworks-sync-for-xero' );?></button>
					&nbsp;
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($){		
		...
		$('#mwqs_cavm_btn').click(function(){
			if(confirm('<?php echo esc_js(__('Are you sure, you want to clear all variation mappings?','myworks-sync-for-xero'))?>')){
				var loading_msg = 'Loading...';
