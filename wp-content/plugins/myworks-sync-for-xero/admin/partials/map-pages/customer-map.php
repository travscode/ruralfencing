...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_customer', 'map_wc_xero_customer' ) ) {	
	...
	if ($table) {
		$wpdb->query("DELETE FROM `" . esc_sql($table) . "` WHERE `X_C_ID` = ''"); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	<div class="card">
		<div class="card-content">
			<div class="row">
				...
				<div class="col col-m">
					<h5><?php esc_html_e( 'Clear all customer mappings', 'myworks-sync-for-xero' );?></h5>
					<?php wp_nonce_field( 'myworks_wc_xero_sync_clear_customer_mappings', 'clear_customer_mappings' ); ?>
					<button id="mwqs_cacm_btn"><?php esc_html_e( 'Clear Mappings', 'myworks-sync-for-xero' );?></button>
					&nbsp;
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($){		
		...
		$('#mwqs_cacm_btn').click(function(){
			if(confirm('<?php echo esc_js(__('Are you sure, you want to clear all customer mappings?','myworks-sync-for-xero'))?>')){
				var loading_msg = 'Loading...';
