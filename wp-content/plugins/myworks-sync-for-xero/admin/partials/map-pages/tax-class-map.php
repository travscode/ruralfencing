...
if ( ! empty( $_POST ) && check_admin_referer( 'myworks_wc_xero_sync_map_wc_xero_tax', 'map_wc_xero_tax' ) ) {
	...
	if(count($item_ids)){
		foreach ($item_ids as $key=>$value){
			...
			//Update
			$eq = $wpdb->prepare("SELECT `id` FROM `" . esc_sql($table) . "` WHERE `wc_tax_id` = %d AND `wc_tax_id_2` = %d ",$save_data['wc_tax_id'],$save_data['wc_tax_id_2']); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	if(count($item_ids_combo)){		
		foreach ($item_ids_combo as $key=>$value){
			...
			if($save_data['wc_tax_id_2'] < 1){
				$wpdb->query($wpdb->prepare("DELETE FROM `" . esc_sql($table) . "` WHERE `wc_tax_id` = %d AND `wc_tax_id_2` > 0 ",$key)); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				continue;
			}				
...
$join = " LEFT JOIN `" . esc_sql($wtr_lt) . "` trl ON (tr.tax_rate_id = trl.tax_rate_id AND trl.location_type = 'city') ";

$search_params = array();
if($tax_map_search!=''){
	$search_like = '%' . $wpdb->esc_like($tax_map_search) . '%';
	$whr.=" AND (tr.`tax_rate_name` LIKE %s OR tr.`tax_rate_class` LIKE %s OR trl.`location_code` LIKE %s) ";
	$search_params = array($search_like, $search_like, $search_like);
}

if(!empty($search_params)){
	$total_records = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM `" . esc_sql($wtr_t) . "` tr " . $join . " WHERE tr.`tax_rate_id` >0 " . $whr, ...$search_params)); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
} else {
	$total_records = $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($wtr_t) . "` tr " . $join . " WHERE tr.`tax_rate_id` >0 " . $whr); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
}
$offset = $MWXS_L->get_offset($MWXS_L->get_page_var(),$items_per_page);

if(!empty($search_params)){
	$tax_q = $wpdb->prepare("SELECT tr.* , trl.location_code FROM `" . esc_sql($wtr_t) . "` tr " . $join . " WHERE tr.`tax_rate_id` >0 " . $whr . " ORDER BY tr.`tax_rate_class` ASC LIMIT %d, %d", ...array_merge($search_params, array($offset, $items_per_page))); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
} else {
	$tax_q = $wpdb->prepare("SELECT tr.* , trl.location_code FROM `" . esc_sql($wtr_t) . "` tr " . $join . " WHERE tr.`tax_rate_id` >0 " . $whr . " ORDER BY tr.`tax_rate_class` ASC LIMIT %d, %d", $offset, $items_per_page); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
}
