...
if($del_queue_id > 0 && $table){
	$wpdb->query($wpdb->prepare("DELETE FROM `" . esc_sql($table) . "` WHERE `id` = %d ",$del_queue_id)); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
...
// Security: Prepare search parameters
$search_params = array();
if(!empty($queue_search)){
	$search_like = '%' . $wpdb->esc_like($queue_search) . '%';
	$whr .= " AND (`item_type` LIKE %s OR `item_action` LIKE %s OR `item_id` = %s)";
	$search_params = array($search_like, $search_like, $queue_search);
}

// Security: Use validated table and proper prepared statements
if ($table) {
	if (!empty($search_params)) {
		$count_query = $wpdb->prepare("SELECT COUNT(*) FROM `" . esc_sql($table) . "` WHERE `id` > 0 " . $whr, $search_params); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	} else {
		$count_query = $wpdb->prepare("SELECT COUNT(*) FROM `" . esc_sql($table) . "` WHERE `id` > 0 " . $whr); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	}
	$total_records = $wpdb->get_var($count_query); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	
	$page = $MWXS_L->get_page_var();
	$offset = ( $page * $items_per_page ) - $items_per_page;
	
	// Security: Use prepared statement for main query
	if (!empty($search_params)) {
		$params = array_merge($search_params, array($offset, $items_per_page));
		$queue_q = $wpdb->prepare("SELECT * FROM `" . esc_sql($table) . "` WHERE `id` > 0 " . $whr . " ORDER BY `id` DESC LIMIT %d, %d", ...$params); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.NotPrepared
	} else {
		$queue_q = $wpdb->prepare("SELECT * FROM `" . esc_sql($table) . "` WHERE `id` > 0 " . $whr . " ORDER BY `id` DESC LIMIT %d, %d", $offset, $items_per_page); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.NotPrepared
	}
} else {
	$total_records = 0;
	$queue_q = "";
