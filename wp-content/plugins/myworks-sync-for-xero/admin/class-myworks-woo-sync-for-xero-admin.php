...
class MyWorks_WC_Xero_Sync_Admin {
	...
	public function create_admin_menus(){
		...
		if($is_license_active){
			$sqm = true;
			if($sqm){
				$q_i_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($MWXS_L->gdtn('queue')) . "` WHERE `id` >0 AND `run` = 0 "); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				add_submenu_page(
					$parent_m_slug,
	...
	private function include_admin_menu_page($page){
		...
		if (!in_array($page, $allowed_pages, true)) {
			...
			error_log('MW Xero Sync Security: Invalid page access attempt: ' . $page);
		...
		if (!file_exists($partial_file) || !is_readable($partial_file)) {
			error_log('MW Xero Sync Security: Attempted to include non-existent file: ' . $partial_file);
		...
		if ($real_path === false || strpos($real_path, $expected_dir) !== 0) {
			error_log('MW Xero Sync Security: Directory traversal attempt detected: ' . $partial_file);
	...
	public function mwxs_ivnt_pull_cron_set(){
		...
		if(is_array($s_rt_pull_items) && !empty($s_rt_pull_items)){
			if(in_array('Inventory',$s_rt_pull_items) || in_array('Product',$s_rt_pull_items) || in_array('Pricing',$s_rt_pull_items) || in_array('Payment',$s_rt_pull_items)){
				$ppi_d_options = $MWXS_L->pp_interval_d_options();
	...
	public function mwxs_process_queue(){
		...
		if ($rate_limits['should_delay']) {
			$queue_limit = " LIMIT 5"; // Process fewer items when approaching limit
			...
			$queue_limit = " LIMIT 20"; // Normal processing limit
		}
		...
		$sql = "SELECT * FROM `" . esc_sql($table) . "` WHERE `run` = 0 ORDER BY `added_date` ASC" . $queue_limit;
			...
			foreach($queue_data as $data){
				...
				if($item_type=='Order' && $item_action=='Push'){
					if(!in_array($item_id,$order_ids)){
						// Rate limit check and delay before processing
						if ($MWXS_L->should_delay_api_call()) {
							sleep(2); // 2 second delay between API-heavy operations
						}
				...
				if($item_type=='Refund' && $item_action=='Push'){
					if(!in_array($item_id,$refund_ids)){
						...
						if($this->hook_refund_add(array('refund_id'=>$item_id,'order_id'=>$order_id,'f_q_f'=>true))){
							$success = 1;
						}
	...
	public function mwxs_admin_head(){
		echo '<style>#adminmenu #toplevel_page_myworks-wc-xero-sync.wp-not-current-submenu .wp-has-submenu div.wp-menu-image img{padding: 12px 0 0; opacity: 1; max-width: 80%;}</style>';
	}
			...
			if(is_array($invoice_data) && !empty($invoice_data)){
				...
				if($order_sync_as == 'Invoice'){
					...
					if(is_object($xero_invoice) && !empty($xero_invoice)){
						...
						echo '<span class="mw_qbo_sync_status_span mw_qbo_sync_status_info">'.esc_html($xero_invoice->getInvoiceNumber()).'</span>';
						echo '</p>';

						echo '<p><a href="javascript:void(0);"><button id="mwxs_odp_ptx" class="button" type="button">Update in Xero</button></a></p>';

						$invoice_id = $xero_invoice->getInvoiceID();
						...
						echo '<p>';
						echo '<a target="_blank" href="'.esc_url($vix_href).'" title="'.esc_attr($vix_title).'"><button class="button" type="button">View</button></a>';
						...
						echo '<a target="_blank" href="'.esc_url($pdf_url).'" title="'.esc_attr($pdf_title).'" style="padding-left: 5px;"><button class="button" type="button">PDF</button></a>';
						echo '</p>';
						...
						echo '<p><a href="javascript:void(0);"><button id="mwxs_odp_ptx" class="button" type="button">Push to Xero</button></a></p>';
					}

					$sync_window_url = $MWXS_L->get_sync_window_url();
					echo '<script>';
					echo 'if (typeof popUpWindow === "undefined") {';
					echo 'function popUpWindow(URLStr, popUpWin, left, top, width, height) {';
					echo 'width = 750; height = 480;';
					echo 'left = (screen.width/2)-(width/2); top = (screen.height/2)-(height/2);';
					echo 'window.open(URLStr, popUpWin, "toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=" + width + ",height=" + height + ",left=" + left + ",top=" + top);';
					echo '}';
					echo '}';
					echo 'jQuery(document).ready(function($) {';
					echo '$(\'#mwxs_odp_ptx\').click(function(){';
					echo 'popUpWindow(\''.esc_url_raw($sync_window_url).'&sync_type=push&item_ids='.esc_js($order_id).'&item_type=order\',\'mw_xs_order_push\',0,0,650,350);';
					echo '});';
					echo '});';					
					echo '</script>';
				}
		...
		switch($column){
			case 'c_xs_mwxs' :
				if($order_id > 0){
					...
					echo '<div id="c_xs_'.esc_attr($order_id).'" class="c_xs_data">...</div>';
				}
				break;
		}
	}
	...
	public function hook_order_add($order_info){
		...
		if($order_id > 0){
			if(!$MWXS_L->lp_chk_osl_allwd()){
				$error_data = array('type'=>'Order','title'=>'Create Order Error #'.$order_id,'details'=>'Monthly limit of orders synced has been reached. To manage your plan and view more details, visit MyWorks Sync > Connection.','status'=>0);
