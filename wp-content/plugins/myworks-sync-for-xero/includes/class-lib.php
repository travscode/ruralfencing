...
class MyWorks_WC_Xero_Sync_Lib extends MyWorks_WC_Xero_Sync_Core {
		...
		if(!$this->is_xero_connected){
			...
			if(!empty($this->AccessToken) && !empty($this->XeroTenantId)){
				...
				$stack->push(GuzzleHttp\Middleware::mapResponse(function ($response) {
					...
					$this->add_wc_debug_log(print_r($headers, true), true, false, false);
	...
	protected function myworks_wc_xero_sync_check_license($licensekey,$localkey,$realtime){
		...
		if(!empty($lc_url)){
			...
			if(!empty($licensekey) && !empty($site_url) && strlen($licensekey) == '35' && $this->validate_connection_key($licensekey)){
				// Security: Use constant instead of hardcoded key
				$licensing_secret_key = defined('MW_WC_XERO_SYNC_LICENSING_SECRET_KEY') ? MW_WC_XERO_SYNC_LICENSING_SECRET_KEY : '';
				$localkeydays = 5;
				...
				$random_number = wp_rand(1000000000, 9999999999);
	...
	public function get_log_chart_data(){
		...
		$result_inv_today = $this->get_data($wpdb->prepare($query_today, $today, '%Draft Invoice not allowed%')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		$result_inv_month = $this->get_data($wpdb->prepare($query_month, $month, '%Draft Invoice not allowed%')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		$result_inv_year = $this->get_data($wpdb->prepare($query_year, $year, '%Draft Invoice not allowed%')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		$result_pmnt_today = $this->get_data($wpdb->prepare($query_pmnt_today, $today)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		$result_pmnt_month = $this->get_data($wpdb->prepare($query_pmnt_month, $month)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		$result_pmnt_year = $this->get_data($wpdb->prepare($query_pmnt_year, $year)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function AutoMapCustomers($cam_wf,$cam_qf,$mo_um=false){
		...
		$sql = $wpdb->prepare('SELECT ' . $wpdb->users . '.ID, ' . $wpdb->users . '.display_name, ' . $wpdb->users . '.user_email FROM ' . $wpdb->users . ' INNER JOIN ' . $wpdb->usermeta . ' ON ' . $wpdb->users . '.ID = ' . $wpdb->usermeta . '.user_id WHERE ' . $wpdb->usermeta . '.meta_key = %s AND (' . implode(' OR ', $placeholders) . ')', array_merge(array($capabilities_key), $values)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		if($cam_qf=='first_last'){
			...
			if(!isset($allowed_fields[$cam_qf])) {
				error_log('MW Xero Sync Security: Invalid column name: ' . $cam_qf);
				return $map_count;
			}
		...
		if(!$validated_x_customer_tbl) {
			error_log('MW Xero Sync Security: Invalid customer table name');
			return $map_count;
		}
	...
	public function check_save_automap_customer_data($w_cus,$all_qbo_customers,$cam_wf,$cam_qf,$mo_um=false){
		...
		if($mo_um){
			$ID = $w_cus['ID'];							
			$e_mr = $this->get_row($wpdb->prepare("SELECT `id` FROM {$map_tbl} WHERE `W_C_ID` = %d ",$ID)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			if(!empty($e_mr)){
				return;
			}
		}
	...
	public function AutoMapProducts($pam_wf,$pam_qf,$mo_um=false){
		...
		if(!$mo_um){
			$wpdb->query("DELETE FROM `".$map_tbl."` WHERE `id` > 0 "); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$wpdb->query("TRUNCATE TABLE `".$map_tbl."` "); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	public function check_save_automap_product_data($w_pro,$all_qbo_products,$pam_wf,$pam_qf,$mo_um=false){
		...
		if($mo_um){
			$ID = $w_pro['ID'];
			$e_mr = $this->get_row($wpdb->prepare("SELECT `id` FROM {$map_tbl} WHERE `W_P_ID` = %d ",$ID)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			if(!empty($e_mr)){
				return;
			}
		}
	...
	public function AutoMapVariations($vam_wf,$vam_qf,$mo_um=false){
		...
		$mml = 500;
		$amc = (int) $wpdb->get_var($sql_c); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function check_save_automap_variation_data($w_pro,$all_qbo_products,$vam_wf,$vam_qf,$mo_um=false){
		...
		if($mo_um){
			$ID = $w_pro['ID'];
			$e_mr = $this->get_row($wpdb->prepare("SELECT `id` FROM {$map_tbl} WHERE `W_V_ID` = %d ",$ID)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			if(!empty($e_mr)){
				return;
			}
		}
	...
	private function clear_invalid_mappings_by_loop($list_table,$map_table,$it_id_field,$mt_id_field,$it_qb_id_field,$mt_qb_id_field){
		...
		if(!$validated_list_table || !$validated_map_table) {
			error_log('MW Xero Sync Security: Invalid table names in clear_invalid_mappings_by_loop');
			return 0;
		}
		...
		if(is_array($wc_items) && !empty($wc_items)){
			...
			if(empty($map_data)){				
				if($wc_variation_id > 0){
					$map_data = $this->get_row($wpdb->prepare("SELECT `X_P_ID` AS ItemID FROM `".$this->gdtn('map_variations')."` WHERE `W_V_ID` = %d AND `X_P_ID` !='' ",$wc_variation_id)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				}
			}
			
			if(empty($map_data) && $wc_product_id > 0){				
				$map_data = $this->get_row($wpdb->prepare("SELECT `X_P_ID` AS ItemID FROM `".$this->gdtn('map_products')."` WHERE `W_P_ID` = %d AND `X_P_ID` !='' ",$wc_product_id)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared			
			}
	...
	public function if_xero_product_exists($product_data, $omc=false){
		if(is_array($product_data) && !empty($product_data)){
			...
			$validated_map_tbl = $this->get_validated_table_name_from_full($map_tbl);
			if(!$validated_map_tbl) {
				error_log('MW Xero Sync Security: Invalid map table name');
				return '';
			}
	...
	public function if_xero_customer_exists($customer_data,$omc=false,$cbn=false,$rt_check=true,$r_obj=false){
		if(is_array($customer_data) && !empty($customer_data)){
			...
			$validated_map_tbl = $this->get_validated_table_name_from_full($map_tbl);
			if(!$validated_map_tbl) {
				error_log('MW Xero Sync Security: Invalid customer map table name');
				return '';
			}
			...
			do{
				...
				if(!empty($result)){
					...
					if(is_array($contacts) && !empty($contacts)){
						...
						if($page == 1){
							// Security: Validate table name and use esc_sql
							$validated_tbl = $this->get_validated_table_name_from_full($tbl);
							if($validated_tbl) {
								$wpdb->query("DELETE FROM `" . esc_sql($validated_tbl) . "` WHERE id > 0");
								$wpdb->query("TRUNCATE TABLE `" . esc_sql($validated_tbl) . "`");
							}
						}						
	...
	public function xero_get_tracking_categories_kva(){
		if($this->is_xero_connected()){
			$X_Whr = 'Status=="ACTIVE"';
			$TrackingCategories = $this->X_API_I()->getTrackingCategories($this->XeroTenantId,$X_Whr,null,null);
			if(!empty($TrackingCategories) && is_array($TrackingCategories->getTrackingCategories()) && !empty($TrackingCategories->getTrackingCategories())){
				$X_TrackingCategories_Kva = array();
				foreach($TrackingCategories->getTrackingCategories() as $TrackingCategory){
					$X_TrackingCategories_Kva[$TrackingCategory->getTrackingCategoryID()] = $TrackingCategory->getName();
				}
				return $X_TrackingCategories_Kva;
			}
		}
	...
	public function xero_get_tracking_options_kva(){
		if($this->is_xero_connected()){
			$X_Whr = 'Status=="ACTIVE"';
			$TrackingCategories = $this->X_API_I()->getTrackingCategories($this->XeroTenantId,$X_Whr,null,null);
			if(!empty($TrackingCategories) && is_array($TrackingCategories->getTrackingCategories()) && !empty($TrackingCategories->getTrackingCategories())){
		...
		if($this->is_xero_connected()){
			...
			if(!empty($s)){
				...
				if(is_array($arr) && count($arr) == 2){
					...
					$result = $this->X_API_I()->getTrackingCategory($this->XeroTenantId,$TrackingCategoryID);
					...
					if(!empty($result)){
						...
						if(is_array($xr_credit_notes) && !empty($xr_credit_notes)){
							...
							if(!$x_credit_note->getHasErrors()){
								...
								$ld = $this->get_error_message_from_xero_validation_errors($validation_errors);
								if(!empty($ld)){						
									$this->save_log(array('type'=>$lt,'title'=>'Create '.$ltt.' Error #'.$refund_id,'details'=>$ld,'status'=>0),true);
								}								
	...
	public function X_Add_Update_Invoice($invoice_data, $is_update=false, $xero_invoice_object=null){
		if(is_array($invoice_data) && !empty($invoice_data)){
			if($this->is_xero_connected()){
				if(!$is_update && $xero_invoice_id = $this->if_xero_invoice_exists($invoice_data)){
					return false;
				}
				...
				if(empty($X_ContactID)){
					$this->save_log(array('type'=>$lt,'title'=>$log_sync_type.' '.$lt.' Error #'.$ord_id_num,'details'=>'Xero customer not found','status'=>0));
					return false;
				}
				...
				if($is_update){
					...
					if($Invoice->getContact()->getContactId() != $X_ContactID){
						$this->save_log(array('type'=>$lt,'title'=>$log_sync_type.' '.$lt.' Error #'.$ord_id_num,'details'=>'Customer Mismatch','status'=>0));
						return false;
					}
						...
						if(!empty($result)){
							...
							if(is_array($xr_invoices) && !empty($xr_invoices)){
								...
								if(!$x_invoice->getHasErrors()){
									...
									$ld = $this->get_error_message_from_xero_validation_errors($validation_errors);
									if(!empty($ld)){						
										$this->save_log(array('type'=>$lt,'title'=>'Update '.$ltt.' Error #'.$ord_id_num,'details'=>$ld,'status'=>0),true);
									}
	...
	public function UpdateWooCommercePrice($ItemID,$UnitPrice,$item_data=array()){
		if(!empty($ItemID)){
			global $wpdb;
			$map_data = $this->get_data($wpdb->prepare("SELECT `W_P_ID` FROM `".$this->gdtn('map_products')."` WHERE `X_P_ID` = %s AND `W_P_ID` > 0 ",$ItemID)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$is_variation = false;
			if(empty($map_data)){
				$map_data = $this->get_data($wpdb->prepare("SELECT `W_V_ID` FROM `".$this->gdtn('map_variations')."` WHERE `X_P_ID` = %s AND `W_V_ID` > 0 ",$ItemID)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				$is_variation = true;
			}
