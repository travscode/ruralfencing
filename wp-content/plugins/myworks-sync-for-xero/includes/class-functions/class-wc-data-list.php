...
class MyWorks_WC_Xero_Sync_Wc_Data_List extends MyWorks_WC_Xero_Sync_Core {
	...
	public function get_dashboard_status_data(){
		...
		if ($customers_table) {
			$x_customer_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($customers_table) . "`"); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}
		
		if ($products_table) {
			$x_product_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($products_table) . "`"); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}
		...
		$data['customer_mapped'] = ($map_customers_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($map_customers_table) . "` WHERE `X_C_ID` !='' ") : 0; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		
		$data['product_mapped'] = ($map_products_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($map_products_table) . "` WHERE `X_P_ID` !='' ") : 0; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		
		$data['variation_mapped'] = ($map_variations_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($map_variations_table) . "` WHERE `X_P_ID` !='' ") : 0; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		
		$data['payment_gateways_mapped'] = ($map_payment_method_table) ? (int) $wpdb->get_var("SELECT COUNT(*) FROM `" . esc_sql($map_payment_method_table) . "` WHERE `X_ACC_ID` !='' ") : 0; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public function get_product_type_by_id($product_id){
		...
		if($product_id>0){
			...
			INNER JOIN `" . esc_sql($wpdb->term_relationships) . "` AS term_relationships ON p.ID = term_relationships.object_id
			INNER JOIN `" . esc_sql($wpdb->term_taxonomy) . "` AS term_taxonomy ON term_relationships.term_taxonomy_id = term_taxonomy.term_taxonomy_id
			INNER JOIN `" . esc_sql($wpdb->terms) . "` AS terms ON term_taxonomy.term_id = terms.term_id
	...
	public function get_wc_customers($is_count=false,$list_page=false,$limit='',$search_txt='',$cl_role_search='',$cl_um_srch='') {
		...
		if($search_txt!=''){
			...
			if (!empty($s_c_ids)) {
				...
				if (!empty($id_array)) {
					...
					$query_template = " OR `" . esc_sql($wpdb->users) . "`.ID IN (" . $id_placeholders . ")"; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$c_id_w = $wpdb->prepare($query_template, ...$id_array); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				}
			}
			
			$search_query_template = " AND (`" . esc_sql($wpdb->users) . "`.display_name LIKE %s OR `" . esc_sql($wpdb->users) . "`.user_email LIKE %s OR `" . esc_sql($wpdb->users) . "`.ID = %s) ";
			$ext_whr .= $wpdb->prepare($search_query_template, $search_like, $search_like, $search_txt) . $c_id_w; // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
		...
		#echo $sql;
		// Security: Prepare the query with capabilities meta_key parameter
		$capabilities_key = $wpdb->prefix . 'capabilities';
		$prepared_sql = $wpdb->prepare($sql, $capabilities_key, $capabilities_key); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		if($is_count){
			...
			if(is_array($q_data) && !empty($q_data)){
				foreach($q_data as $rd){
					...
					$mq = $wpdb->prepare("SELECT X_C_ID FROM `" . esc_sql($mt) . "` WHERE W_C_ID = %d AND X_C_ID != ''", $rd['ID']); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$md =  $this->get_row($mq);
					if(is_array($md) && !empty($md) && !empty($md['X_C_ID'])){
						$X_C_ID = $md['X_C_ID'];
						$xct = $this->gdtn('customers');
						$xcq = $wpdb->prepare("SELECT Name, EmailAddress FROM `" . esc_sql($xct) . "` WHERE ContactID = %s", $X_C_ID); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public function get_wc_order_list($is_count=false,$limit='',$search_txt='',$date_from='',$date_to='',$status=''){
		...
		if ($this->is_hpos_enabled()) {
			return $this->get_wc_order_list_hpos($is_count,$limit,$search_txt,$date_from,$date_to,$status);
		}
	...
	public function get_wc_order_list_hpos($is_count=false,$limit='',$search_txt='',$date_from='',$date_to='',$status=''){
		global $wpdb;
